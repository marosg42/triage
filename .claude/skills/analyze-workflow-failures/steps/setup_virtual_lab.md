...
- **v1.5** (2026-05-27): Fourth confirmation of the tunnel-drop-after-initial-success pattern from run 26444927960 / UUID 73a69f8d-10c0-4d23-b889-d3c8db2c3a79 — relay `10.241.3.48` started `infra1/2/3`, SSH reachability to `10.241.144.2/.3/.4` succeeded, `generated/sshtest.txt` captured one successful `infra1` probe followed by `Connection closed by 10.241.144.2 port 22`, and `maas login` degraded from `IncompleteRead(380435 bytes read, 158769 more expected)` to repeated `[Errno 113] No route to host` before failing after 10 attempts.
- **v1.4** (2026-05-27): Added one-node SSH validation failure pattern from run 26444917865 / UUID 6074c4cd-b457-489c-9805-1b68eff262e4 — relay `10.241.3.43` and `virsh start infra1/2/3` succeeded, `sshuttle` reached `10.241.144.2` and `.3`, but `10.241.144.4` repeatedly returned `Connection closed by remote host` and never passed `ssh-keyscan`, so `setup-tunnel` exited before `maas login`.
- **v1.3** (2026-05-27): Third confirmation of the tunnel-drop-after-initial-success pattern from run 26444924273 / UUID 40f03deb-1fcf-4f83-8ad3-055125ddac10 — relay `10.241.6.29` started `infra1/2/3`, SSH reachability to `10.241.144.2/.3/.4` succeeded at 10:26:43Z, `maas login` then degraded from `IncompleteRead(5954 bytes read, 533250 more expected)` to repeated `[Errno 113] No route to host`, and `generated/sshtest.txt` captured one successful probe followed by `Connection closed by 10.241.144.2 port 22`.
- **v1.2** (2026-05-27): Confirmed tunnel-drop-after-initial-success pattern with run 26444920976 / UUID 5147eb17-151f-4eb5-b59f-eaed68901db8 — relay `10.241.4.13` started `infra1/2/3`, SSH reachability to `10.241.144.2/.3/.4` eventually succeeded, then `maas login` degraded from HTTP 502 to `IncompleteRead` to repeated `[Errno 113] No route to host`; `generated/sshtest.txt` captured one successful probe followed by `Connection closed by 10.241.144.2 port 22`.
- **v1.1** (2026-05-27): Added tunnel-drop-after-initial-success pattern from run 26444934886 / UUID fdb17b26-7267-48db-b674-ccb0b863055b — SSH scans for `10.241.144.2/.3/.4` succeeded, but `generated/sshtest.txt` showed the tunnel died immediately afterwards and `maas login` degraded from HTTP 502 to `IncompleteRead` to repeated `[Errno 113] No route to host`.
- **v1.0** (2026-05-27): Initial version. Added tunnel-preflight failure pattern where `virtual_maas` relay setup succeeds but `10.241.144.2` never becomes SSH-reachable, from run 26503617854 / UUID 856857aa-ec36-40d5-adfc-03b67a0b5959.
