...
- **v1.0** (2026-03-27): Initial version — AppArmor virsh pkttyagent denial pattern from
  run 23321492075 (UUID 8faad6c3, tor3-sqa-virtual_maas cluster_5).
- **v1.2** (2026-04-02): Added Pattern C — Curtin `install_kernel` fails with "Unable to
  locate package linux-generic": apt-get update makes zero network requests (no Squid
  proxy activity in ~250ms window), leaving package database empty; `netboot_off` never
  called; nodes PXE-loop for 30 minutes; from run 23910974753 (UUID f8251919,
  tor3-sqa-virtual_maas cluster_6, Curtin 23.1.1-1124-g7324b43b, noble/20260223).
- **v1.3** (2026-04-06): Added rev 41764 as a second affected revision for Pattern A (AppArmor virsh pkttyagent denial); confirmed from run 23959562380 (UUID fde2213c, tor3-sqa-dedicated_maas dh1_j2, 2026-04-03). Same profile defect as 41649: `snap.maas.pebble` denies `virsh` exec of `/usr/bin/pkttyagent` and kills virsh via seccomp (SECCOMP_RET_KILL_PROCESS, `code=0x50000`) for syscalls 122/123.
- **v1.5** (2026-04-04): Added Pattern D — Curtin `installing-missing-packages` fails with "Unable to locate package efibootmgr/grub-efi-amd64/shim-signed" (exit 100 in <200ms): noble squashfs `5f64c83` has empty APT package index; Temporal retries each take ~5 min; 30-min FCE timeout exhausted after 4–5 retry cycles; from run 23969669421 (UUID 16f351cf, tor3-sqa-dedicated_maas dh1_j2, Curtin 23.1.1-1124-g7324b43b, nodes suicune + pangoro).
- **v1.6** (2026-04-06): Pattern C confirmed second occurrence — run 23936994907 (UUID 72dc244a, tor3-sqa-virtual_maas cluster_5, squashfs 5f64c83, 2026-04-03); same 64ms apt-get update, 17 PXE cycles/node, concurrent AppArmor DENIED for snap.maas.pebble rev 41764 virsh→pkttyagent (secondary, does not affect primary failure mechanism).
- **v1.7** (2026-04-08): Added Pattern E — Ubuntu security mirror package replacement race: `python3-django_4.2.11-1ubuntu1.15` returns HTTP 404 during `apt-get install maas-region-controller`; all other 110 packages succeed; apt exits code 100; transient CDN transition between `apt-get update` and `apt-get install`; from run 24111793277 (UUID fc17932b, tor3-sqa-virtual_maas cluster_7, 2026-04-08).
- **v1.8** (2026-04-08): Added Pattern F — boot resource import fails (internal mirror unreachable): FCE sets boot source to `http://10.141.186.167/maas/images/ephemeral-v3/stable/`; infra KVM VMs cannot route to this IP (`[Errno 113] No route to host`); two CRITICAL import failures logged; FCE's `list_boot_images` check fooled by stale rack-controller TFTP cache from previous runs; region DB has no boot resources; `machines create` fails immediately with empty architecture list (`'It should be one of: ''.`); from run 24122101622 (UUID 01dd77f9, tor3-sqa-virtual_maas cluster_6, MAAS 3.6.4/snap rev 41799, 2026-04-08).
