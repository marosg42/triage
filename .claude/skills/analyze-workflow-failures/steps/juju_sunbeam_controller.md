...
- **v1.8** (2026-06-02): Pattern 1 confirmed again in run 26662393808 / UUID cde3f622-1f89-49ca-b183-39d0a69c1522 — `admin.openrc` targeted `https://10.241.38.134/openstack-keystone/v3` with `OS_INTERFACE=public`, `generated/sunbeam/kubectl_get_svc.txt` mapped that IP to `traefik-public-lb`, runner-side refstack validation still returned repeated `200` responses from that VIP at 02:29, `sshuttle_metallb_range.txt` stayed UP through the 02:39 failure window, the bootstrap VM acquired tenant address `192.168.122.10`, and `jujud` then timed out to `10.241.38.134:443` at 02:38:29 on `tor3-sqa-shared_maas` / `dh1_j9_1` (branch `main`).
- **v1.7** (2026-05-25): Pattern 1 confirmed again in run 26330839373 / UUID 63d27721-d330-4801-b773-b5172d068d55 — `admin.openrc` targeted `https://10.241.38.131/openstack-keystone/v3` with `OS_INTERFACE=public`, `generated/sunbeam/kubectl_get_svc.txt` mapped that IP to `traefik-public-lb`, runner-side refstack validation still returned `200` responses from that VIP at 15:32, `sshuttle_metallb_range.txt` remained UP through the 15:42 failure window, the bootstrap VM got `192.168.122.8` and completed cloud-init at 15:40:16, then `jujud` timed out to `10.241.38.131:443` at 15:41:54 on `tor3-sqa-shared_maas` / `dh1_j9_1` (branch `main`).
- **v1.6** (2026-05-25): Pattern 1 confirmed again in run 26330838474 / UUID 088d4286-98cc-4fd9-9bd4-eee460dacf24 — `admin.openrc` targeted `https://10.241.37.131/openstack-keystone/v3` with `OS_INTERFACE=public`, runner-side refstack validation still returned `200` responses from that VIP at 15:31:04–15:31:09, `sshuttle_metallb_range.txt` stayed UP through 15:41, the bootstrap VM acquired tenant address `192.168.122.148` and completed cloud-init at 15:39:17, then `jujud` timed out to `10.241.37.131:443` at 15:39:51 on `tor3-sqa-shared_maas` / `dh1_j8_2` (branch `main`).
- **v1.5** (2026-05-25): Pattern 1 confirmed again in run 26316251543 / UUID 13524d44-b69b-4354-801a-0457aab48cc7 — `admin.openrc` targeted `https://10.241.38.135/openstack-keystone/v3` with `OS_INTERFACE=public`, runner-side refstack validation still returned `200` responses from that VIP at 03:59:51–03:59:58, the bootstrap VM acquired tenant address `192.168.122.65` and completed cloud-init at 04:08:43, then `jujud` timed out to `10.241.38.135:443` at 04:09:19 on `tor3-sqa-shared_maas` / `dh1_j9_1` (branch `main`).
